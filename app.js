(function () {
  ...
  let pageOverflowCheckQueued = false;
  let resumeQualityCheckQueued = false;
  let atsCompatibilityCheckQueued = false;
  let jobMatchCheckQueued = false;
  ...
  const GUIDE_STEPS = [
    {
      ...
      text: 'Create separate resumes, duplicate versions, rename documents, restore saved versions, and keep JSON backups from the document panel.',
    },
    {
    ...
    },
    {
      ...
      text: 'Paste a job description to compare keywords against your resume. ResCapt shows matching and missing terms so you can tailor the resume faster.',
    },
    {
    ...
    },
    {
      ...
      text: 'Add work experience, education, projects, certifications, skills, and a custom section. You can collapse sections and reorder resume sections when needed.',
    },
    {
      selector: '#sectionAppearance',
      title: 'Customize the app and resume',
      text: 'App Theme changes the workspace colors only. Template, font, line spacing, alignment, and color palette change the resume itself.',
    },
    {
    ...
    },
    {
      ...
      text: 'Download PDF creates a preview first. Review the pages, then click Download PDF only when you are happy with the result.',
    },
  ];
  ...
  const ACTION_VERBS = [
    'led', 'built', 'managed', 'improved', 'reduced', 'created', 'launched', 'coordinated',
    'analyzed', 'optimized', 'delivered', 'implemented'
  ];
  ...
  function mergeSampleContent(overrides) {
    ...
    return Object.assign({}, base, overrides, {
      ...
      jobMatch: overrides.jobMatch || { jobDescription: '' },
    });
  }
  ...
  function showSavedStatus() {
    saveStatus.textContent = 'Saved locally in this browser';
  ...
  function schedulePageOverflowCheck() {
    if (pageOverflowCheckQueued) return;
    pageOverflowCheckQueued = true;
    requestAnimationFrame(function () {
      requestAnimationFrame(function () {
        pageOverflowCheckQueued = false;
        updatePageOverflowWarning();
      });
    });
  }
  ...
  function scheduleResumeQualityCheck() {
    if (resumeQualityCheckQueued) return;
    resumeQualityCheckQueued = true;
    requestAnimationFrame(function () {
      resumeQualityCheckQueued = false;
      updateResumeQuality();
    });
  }
  ...
  function updateJobMatch() {
    ...
    if (personalFields.summary.value.trim().length < 120) {
      suggestions.push('Use the summary to name the target role and 1-2 job-post priorities.');
    }
  ...
  function scheduleATSCompatibilityCheck() {
    if (atsCompatibilityCheckQueued) return;
    atsCompatibilityCheckQueued = true;
    requestAnimationFrame(function () {
      atsCompatibilityCheckQueued = false;
      updateATSCompatibility();
    });
  }

  function scheduleJobMatchCheck() {
    if (jobMatchCheckQueued) return;
    jobMatchCheckQueued = true;
    requestAnimationFrame(function () {
      jobMatchCheckQueued = false;
      updateJobMatch();
    });
  }
  ...
  function createBulletSuggestion(card, type) {
    ...
    if (!startsWithActionVerb(suggestion)) {
      suggestion = capitalizeSentence(chooseActionVerb(suggestion, type)) + ' ' + lowerFirst(suggestion);
